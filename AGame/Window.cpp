...
Window_::Window::Window(HINSTANCE exe_start_adress)
{
	...
	mEngine = new GameEngine::Engine(0, 0);
...
void Window_::Window::InitWindow(HINSTANCE exe_start_adress)
{
	...
	wndclass.lpszClassName = L"A Game.";
...
LRESULT Window_::Window::WindowProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_KEYDOWN: 
	{
		...
		if (wp == VK_ESCAPE)DestroyWindow(hwnd);
		if (wp == VK_RIGHT && mEngine && !(lp &(1 << 30))) mEngine->RotateRight();
		if (wp == VK_LEFT && mEngine && !(lp &(1 << 30))) mEngine->RotateLeft();
		return 0;
	}break;
	case WM_PAINT:
	{
	...
	case WM_SIZE:
	{
		...
		if(mEngine)mEngine->Resize(window_size.cx, window_size.cy);
	}return 0;
	...
	{
		//if (wp == 'E')
		//	mEngine->ReCreateResource(0, 0);
		if (wp == 'Q'){
		//	mEngine->PauseApplication();
		//mEngine->ResumeApplication(0, 0);
		}
	}return 0;
	case WM_ACTIVATE:
	{
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			else
			{
				...
				float delta_ms = 0.0f;//timer->GetDeltaTime();
...
{
	/*
	static auto tb = chrono::steady_clock::now();
	static auto tl = chrono::steady_clock::now();
	auto tc = chrono::steady_clock::now();
	//engine->Draw(delta_ms);
	if(mEngine->EndGame(static_cast<float>(chrono::duration_cast<chrono::microseconds>(tc - tl).count()) / (1000.0f), 
		static_cast<float>(chrono::duration_cast<chrono::microseconds>(tc - tb).count()) / (1000.0f)))
		DestroyWindow(g_hWnd);
	mEngine->Draw(static_cast<float>(chrono::duration_cast<chrono::microseconds>(tc - tl).count()) / (1000.0f),
		static_cast<float>(chrono::duration_cast<chrono::microseconds>(tc - tb).count()) / (1000.0f));
	*/
