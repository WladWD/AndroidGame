...
Shader::ShaderBarrierRotor::ShaderBarrierRotor(Resource::ResourceBase *resourceLoad) : ShaderBarrierBase(1)
{
	...
	{
		"resource\\shaders\\barrier\\blade_1\\vertex.glsl",
		"resource\\shaders\\barrier\\blade_1\\fragment.glsl"
	};
...
Shader::ShaderBarrierRotor::~ShaderBarrierRotor() { }

void Shader::ShaderBarrierRotor::InitVariable(GLuint program)
{
	correct_programm
...
GLint Shader::ShaderBarrierRotor::GetProgram(void)
{
	program_mid = 0;
	return programs[0];
}

void Shader::ShaderBarrierRotor::SetProjViewMatrix(glm::mat4 matrix)
{
	correct_programm
...
void Shader::ShaderBarrierRotor::SetWorldMatrix(glm::mat4 matrix)
{
	correct_programm
...
void Shader::ShaderBarrierRotor::SetFog(glm::vec2 fog)
{
	correct_programm
		mcheck(gFog) glUniform2fv(gFog[program_mid], 1, glm::value_ptr(fog));
}

void Shader::ShaderBarrierRotor::SetTexture(void)
{
	glActiveTexture(GL_TEXTURE0 + mTexSlot);
}

void Shader::ShaderBarrierRotor::SetFogColor(const float *color)
{
	correct_programm
		mcheck(gFogColor) glUniform4fv(gFogColor[program_mid], 1, color);
}

GLint Shader::ShaderBarrierRotor::GetPositionAttrib(void)
{
	return glGetAttribLocation(programs[program_mid], "gWorldPos");
}

GLint Shader::ShaderBarrierRotor::GetTexCoordAttrib(void)
{
	return glGetAttribLocation(programs[program_mid], "gTexCoord");
}

void Shader::ShaderBarrierRotor::BindTextureToSlot(void)
{
	correct_programm
