...
Resource::ImageResourceLoad::ImageResourceLoad(std::string mFileName, Resource::ResourceBase *mResourceLoad)
{
	const char *cpRes;
	char *Resource, *pRes;
	uint64_t length;
	...
	mImages = new ImageResource(mTexturesIDs, mGLTexturesIDs, mTextureCount);
...
Resource::ImageResourceLoad::~ImageResourceLoad()
{
}

Resource::GLTextureResource Resource::ImageResourceLoad::ReadImage(char *mResourceTexture, GLuint &mTextureID, uint32_t &mSize)
{
	//lod 0 - ONLY  !!!
...
Resource::ImageResource *Resource::ImageResourceLoad::GetImageResource(void)
{
	return mImages;
}
