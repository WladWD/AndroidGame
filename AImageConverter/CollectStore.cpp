...
Resource::CollectStore::CollectStore(void) : StoreModel()
{
}

Resource::CollectStore::~CollectStore()
{
}

void Resource::CollectStore::WriteCollectParameter(std::string mFileName, const MEngine::MObjects *mObjects, const MeshLoad::StandartMesh *mBuffers)
{
	std::ofstream mof(mFileName, std::ios::binary | std::ios::out);

	SphereBounding mSphere;
	...
	mSphere.mCenterRadius = glm::vec4(mSphereCenter, Radius);
	mof.write(reinterpret_cast<const char *>(&mSphere), sizeof(mSphere));

	mof.close();
}

void Resource::CollectStore::Write(std::string mFileDirectory)
{
	...
	if (firstFile != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			if (CheckFile(mName))
			{
				...
				WriteCollectParameter("out\\" + mLocalPath + mName.substr(0, mName.find_last_of('.')) + ".tdofspc", mLoadModel->GetObjects(), mLoadModel->GetMesh());
