...
Resource::BarrierStore::BarrierStore(void) : StoreModel()
{
}

Resource::BarrierStore::~BarrierStore()
{
}

void Resource::BarrierStore::WriteParameter(std::string mFileName, const MEngine::MObjects *mObjects, const MeshLoad::StandartMesh *mBuffers)
{
	...
	*/
	...
	for (register uint32_t i = 0; i < mDrawObjectCount; i++)
	{
		...
		for (register uint32_t j = mOffset; j < (mDrawParam.mIndexCount + mOffset); j++)
		{
	...
	mAABB.mCenter = (gMaxVec + gMinVec) * 0.5f;
	mAABB.mExtent = (gMaxVec - gMinVec) * 0.5f;

	mof.write(reinterpret_cast<const char *>(&mAABB), sizeof(mAABB));

	mof.close();
}

void Resource::BarrierStore::Write(std::string mFileDirectory)
{
	...
	if (firstFile != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			if (CheckFile(mName))
			{
				...
				WriteParameter("out\\" + mLocalPath + mName.substr(0, mName.find_last_of('.')) + ".tdofaabb", mLoadModel->GetObjects(), mLoadModel->GetMesh());
